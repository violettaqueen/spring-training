...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    List<Account> findByRoleContains(String name);
    ...
    List<Account> retrieveALLAdminAccounts(String role);
    ...
    List<Account>retrieveAllAccountsContainsValue(String value);

    //Write a native query to read all accounts with an age higher than a specific value
    @Query(value = "select * from accountdetailss where age >= value", nativeQuery = true)
    List<Account> retrieveAllAccountsAgeGreaterThan(int value);
