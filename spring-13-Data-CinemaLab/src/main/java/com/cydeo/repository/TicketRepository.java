...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    List<Integer> countAllByUserAccountId(Long userId);  // need to match with the field name = User account

    //Write a derived query to list all tickets by specific email
    List<Ticket> getTicketsByUserAccountEmail(String email);

    //Write a derived query to count how many tickets are sold for a specific movie
    Integer countTicketsByMovieCinemaMovieName(String movieName);

    //Write a derived query to list all tickets between a range of dates
    List<Ticket> getTicketsByDateTimeBetween(@Param("dateTime1")LocalDateTime dateTime1, @Param("dateTime2") LocalDateTime dateTime2);
    ...
    List<Ticket> retrieveAllTicketsDatesBetween(@Param(("dateTime1")) LocalDateTime dateTime1, @Param("dateTime2") LocalDateTime dateTime2);

    // ------------------- Native QUERIES ------------------- //

    //Write a native query to count the number of tickets a user bought
    @Query(value = "select count(*) from ticket where user_account_id= ?1", nativeQuery = true)
    List<Integer> countTicketsByNumberUserBought(@Param("id") Long id);
